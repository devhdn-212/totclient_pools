...
type trxkeluaranApi struct {
	trxkeluaranService domain.TrxkeluaranService
	pasaranService     domain.PasaranService
}

func NewTrxkeluaranApi(app *fiber.App,
	trxkeluaranService domain.TrxkeluaranService,
	pasaranService domain.PasaranService,
	authmidle fiber.Handler) {
	ad := trxkeluaranApi{
		trxkeluaranService: trxkeluaranService,
		pasaranService:     pasaranService,
	}
	trxkeluaran := app.Group("/api/trxkeluaran", authmidle)
	trxkeluaran.Post("", ad.Index)
	trxkeluaran.Post("/save", ad.Save)
}
func (co *trxkeluaranApi) Index(ctx *fiber.Ctx) error {
	...
	resselectpasaran, errpasaran := co.pasaranService.Select(c, idcomp)
	if errpasaran != nil {
		return ctx.Status(http.StatusInternalServerError).
			JSON(dto.CreateResponseError(http.StatusInternalServerError, "internal server error - Pasaran Select"))
	}

	res, err := co.trxkeluaranService.All(c, idcomp)
	if err != nil {
		return ctx.Status(http.StatusInternalServerError).
			JSON(dto.CreateResponseError(http.StatusInternalServerError, "internal server error"))
	}
	return ctx.JSON(fiber.Map{
		...
		"message":     "success",
		"listpasaran": resselectpasaran,
		"record":      res,
	})
}
func (co *trxkeluaranApi) Save(ctx *fiber.Ctx) error {
	c, cancel := context.WithTimeout(ctx.Context(), 10*time.Second)
	defer cancel()

	var req dto.TrxkeluaranSave
	...
	if len(fails) > 0 {
		connection.Log.Warn("Validation failed for update Trxkeluaran",
	...
	err := co.trxkeluaranService.Save(c, req, client_username, idcomp)
	if err != nil {
		recordJson, _ := json.Marshal(req)
		connection.Log.Error("Failed to create / update Pasaran",
			zap.String("id", req.IDcompasaran),
			zap.String("error", err.Error()),
	...
	connection.Log.Info("Trxkeluaran create / update successfully",
		zap.String("id", req.IDcompasaran),
	)
